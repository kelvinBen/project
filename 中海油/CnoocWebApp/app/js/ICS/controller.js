...
systemindexCtrl.$inject = ['$rootScope', '$interval', '$resource', '$scope', '$timeout', '$state', '$stateParams', '$filter', 'eventservice', 'eventperiod', 'eventtables', 'steptask', 'periodtask', 'roletask', 'icsuser', 'eventroles', 'eventorganization', 'eventpublicinfo', 'eventorgusers', 'icsdb', 'operator'];
function systemindexCtrl($rootScope, $interval, $resource, $scope, $timeout, $state, $stateParams, $filter, eventservice, eventperiod, eventtables, steptask, periodtask, roletask, icsuser, eventroles, eventorganization, eventpublicinfo, eventorgusers, icsdb, operator) {
    $scope.showPrimaryEvent = true;
    ...
    $scope.tablestatus = [];
    $scope.meeting = {};
    $scope.status1 = { isFirstOpen: false };
    $scope.status2 = { isFirstOpen: false };
    $scope.status3 = { isFirstOpen: false };
    // $scope.userRole="4";
    ...
    $scope.userid = localStorage.userid;
    icsdb.getdetail("eventorgusers", "?userid=" + localStorage.userid).then(function (dt) {
        if (dt.length == 0) {
            if ($scope.userid == 1) {
                icsdb.getdetail("eventroles", "?roleid=4").then(function (data) {
                    if (data.length > 0) {
                        ...
                        $scope.curUser = {
                            ...
                            role: $scope.curRole.rolename
                        };
                        $scope.curOrg = { organiztionid: $scope.curRole.organizationid };
                        $scope.showPrimaryEvent = $scope.curRole.roleid == "1";
                        $scope.loadDatas();
                    }
                });
            } else return;
        ...
        var data = [];
        angular.forEach(dt, function (d) {
            if (d.companyid != "c435509cf11c6abd") data.push(d);
        });
        // var role = data[0];
        // angular.forEach(data,function(item){
        ...
        // });
        if (data.length > 0) {
            icsdb.getdetail("eventroles", "?roleid=" + data[0].roleid).then(function (data) {
                if (data.length > 0) {
                    $scope.curRole = data[0];
                    $scope.curUser = {
                        userid: $scope.userid,
                    ...
                    $scope.showPrimaryEvent = $scope.curRole.roleid == "1";
                    $scope.loadDatas();
                    // icsdb.getdetail("eventorganization", "?id=" + data[0].organization).then(function (os) {
                    //     if (os.length > 0) {
                    //         $scope.curOrg = os[0];
                    //         $scope.loadDatas();
                    //     }
    ...
    $scope.corrds = ["107,1,335,40", "107,77,335,166", "107,204,335,255", "107,289,335,354", "107,398,335,450", "107,490,335,525", "107,590,335,634", "107,666,335,708"];
    ...
    $scope.getgroupworks = function () {
        ...
        var userRole = $scope.curRole.roleid;
    ...
    var refFun = $scope.loadDatas = function () {
        //localStorage.belongs = $scope.curBelongs;
        if (!$scope.curOrg || !$scope.curOrg.organiztionid)
            return;
        $scope.events = [];
        eventservice.get().then(function (eventdatas) {
            var data = [];
            angular.forEach(eventdatas, function (eventItem) {
    ...
    // };
    if ($stateParams.type == null) {//自动刷新
        // setInterval($scope.loadDatas(), )  
        icsdb.get('systemconfig').then(function (dt) {
            if (dt && dt.length > 0) {
                var time = dt[0].refreshtime;
                if (isNaN(time)) $interval(refFun, 1000 * 60 * parseInt(time));
                else $interval(refFun, 1000 * 10 * 60);
            }
        });
    }
}
            ...
            if ($scope.curevent.organiztion == "81940c2be8212921") {
                ...
                eventservice.update($scope.curevent.id, $scope.curevent).then(function (data) {
                    ...
                    if ($scope.sendMsg) {
                        ...
                        var content = $scope.vm.data.meetingname + "<br />";
                        content += new moment($scope.vm.data.dt).format("YYYY-MM-DD HH:mm:ss") + "<br />";
                        content += $scope.vm.data.locate + "<br />";
                        var obj = {
                            content: content,
                            title: $filter('T')("emergency"),
                            category_type: "3",
                            type: "3",
                            top: 0,
                            is_return: 0,
...
function primarytableCtrl($rootScope, $resource, $scope, $timeout, $state, $stateParams, $filter, eventservice, eventperiod, eventtables, users, eventorgusers, eventorganization, countryrisk, countrycity, eventroles, icsdb, tablecontent) {
    ...
    $scope.addevent = function () {
        ...
        $scope.vm_event.name = $scope.vm_event.name + '(' + $filter('date')($scope.dt, 'yyyy/MM/dd') + ')';
...
organiztionCtrl.$inject = ['$rootScope', '$resource', '$scope', '$timeout', '$state', '$stateParams', '$filter', 'eventservice', 'eventperiod', 'icsdb'];
function organiztionCtrl($rootScope, $resource, $scope, $timeout, $state, $stateParams, $filter, eventservice, eventperiod, icsdb) {

    // $scope.eventid = localStorage.eventid;
    eventservice.getdetail(localStorage.eventid).then(function (data) {
        // $scope.vm = data;
        icsdb.getdetail('eventorganization', data.organiztion).then(function (dt) {
            $scope.vm = dt;
        });
    });
...
function eventtablelistCtrl($rootScope, $resource, $scope, $timeout, $state, $filter, eventservice, eventperiod, eventtables, steptask, periodtask, users) {
    ...
    $scope.filtertables = function () {
        ...
        var ary = [];
        angular.forEach($scope.alltables, function (item, index) {
            if (item.eventid == $scope.curevent.id && item.tablename != "meeting") {
...
function ics234Ctrl($rootScope, $resource, $scope, $timeout, $state, $stateParams, $filter, eventservice, eventperiod, eventtables, users, icsdb) {
    ...
    $scope.getstrategy = function (item2) {
        var strategy = "";
        angular.forEach(item2.data, function (item3, index3) {
            if (item3.checked == true && item3.status != "完成") {
                strategy += item3.name + "; ";
                return;
            }
        });
                        ...
                        if (item3.checked == true && item3.status != "完成") {
                            if (item3.person && item3.person.orgid) {
                                if (item3.person.orgid == $scope.vm.orgid) {
                                    $scope.goals.push({
                                        goalindex: index2,
                                        strategyindex: index3,
                                        name: item2.taskname + "(" + item3.name + ")",
                                        workarrangement: item3.workarrangement,
                                        user: item3.user
                                    });
                                }
                            }
...
publicCtrl.$inject = ['$resource', '$scope', '$filter', '$state', '$timeout', 'pubsentiment'];
function publicCtrl($resource, $scope, $filter, $state, $timeout, pubsentiment) {
    $scope.eventRange = {
    ...
    $scope.search = function () {
        var st = $scope.eventRange.startDate.format("YYYY-MM-DD");
        var et = $scope.eventRange.endDate.format("YYYY-MM-DD");
        pubsentiment.getWithTime(st, et).then(function (data) {
            $scope.items = data;
        });
    };

    $scope.send = function (evt) {
        var content = evt.title + "<br />";
        content += evt.abstracts;
        var obj = {
            content: content,
...
ics2012Ctrl.$inject = ['$rootScope', '$resource', '$scope', '$timeout', '$state', '$stateParams', '$filter', 'eventservice', 'eventperiod', 'eventtables', 'tablecontent', 'users', 'icsdb', 'FileUploader', 'SweetAlert'];
function ics2012Ctrl($rootScope, $resource, $scope, $timeout, $state, $stateParams, $filter, eventservice, eventperiod, eventtables, tablecontent, users, icsdb, FileUploader, SweetAlert) {
    ...
    $scope.changetask = function () {
        ...
        //$scope.vm.data.actions = $scope.vm.data.currentaim.actions;
    };
    ...
    $scope.gettasks = function () {
        icsdb.getdetail("eventstatus", "?eventid=" + $scope.vm.eventid).then(function (data) {
            if (data.length > 0) {
                ...
                angular.forEach(goals, function (item2, index2) {
                    angular.forEach(item2.data, function (item3, index3) {
                        if (item3.checked == true && item3.user && item3.user.user_id == localStorage.userid) {
                            if (!item3.taskname) {
                        ...
                        // if (item3.checked == true && item3.person && item3.person.orgid == $scope.roleMd.organization){
                        //     if (!item3.taskname) {
                        //         item3.taskname = item2.name + "(" + item3.name + ")";
                        //     }
                        //     item3.user = $scope.userMd;
                        //     $scope.tasks.push(item3);
                        //     return;
                        // }
                    });
    ...
    var ischildroleid = function (userRole, roleid) {
        ...
        else if ((userRole == "36" || userRole == "43") && roleid == "44") {
            return true;
        }
    ...
    $scope.submit = function (flag) {
        $scope.vm.time = $filter('date')($scope.dt, 'yyyy-MM-dd HH:mm');
        $scope.vm.data.files = $scope.files;
        $scope.vm.datajson = angular.toJson($scope.vm.data);
        eventtables.
            add($scope.vm).then(function (data) {
                if ($scope.vm.data.currentaim) {
                    ...
                    icsdb.getdetail("eventstatus", "?eventid=" + $scope.vm.eventid).then(function (data2) {
                        if (data2.length > 0) {
                            angular.copy($scope.vm.data.task, data2[0].task);
                            icsdb.update("eventstatus", data2[0].id, data2[0]).then(function () {
                                $scope.updateEventInfos();
                                if (flag == true) {
                                    $scope.subminEx();
                                }
                            });
    ...
    if (pdata) {
        ...
        if (pdata.id) {
            eventtables.getdetail(pdata.id).then(function (data) {
                ...
                $scope.dt = new Date();
                $scope.vm.data = angular.fromJson($scope.vm.datajson);
                if ($scope.vm.data && $scope.vm.data.files) {
                    $scope.files = $scope.vm.data.files;
                }
                eventservice.getdetail(data.eventid).then(function (data2) {
                    $scope.vm.name = data2.name;
                });
    ...
    //更新当前动态
    $scope.updateEventInfos = function () {
        icsdb.getdetail("eventstatus", "?eventid=" + $scope.vm.eventid).then(function (data) {
            if (data.length > 0) {
                $scope.bShowActionDuty = false;
                if (data[0].actiondutyurl) {
                ...
                var msg = "";
                angular.forEach($scope.event_goals, function (goal, index3) {
                    if (goal.starttime) msg += ' ' + goal.starttime;
                    if (goal.username) msg += goal.name;
                    msg += goal.name + '(' + goal.strategy + ') <br />';
                });
                icsdb.getdetail('eventpublicinfo', '?eventid=' + $scope.vm.eventid).then(function (data) {
                    if (data && data.length == 1) {//edit
                        var obj = data[0];
                        obj.content += '<br />' + msg;
                        icsdb.update('eventpublicinfo', obj.id, obj);
                    } else if (data && data.length == 0) {//add new 
                        var obj = {
                            id: '',
    ...
    $scope.uploaderCtl.onSuccessItem = function (fileItem, response, status, headers) {
        if (response.eCode == 0) {
            var obj = {
                filename: response.filename.replace(/"/g, ""),
                url: response.url
            };
...
icsfilesCtl.$inject = ['$scope', '$filter', 'icsdb', '$state', 'SweetAlert', 'eventorganization'];
function icsfilesCtl($scope, $filter, icsdb, $state, SweetAlert, eventorganization) {
    $scope.typeList = ['应急培训要求', '培训记录', '主要人员应急能力', '应急预案', '处置方案', '应急演练记录', '其他'];
    ...
    $scope.loadEventOrg();
    $scope.search = function () {
        icsdb.get('eventfiles', '?institutions=' + $scope.selOrg.id).then(function (data) {
            var temp = [];
...
addicsfileCtl.$inject = ['$scope', '$filter', '$stateParams', 'icsdb', '$state', 'SweetAlert', 'eventorganization', 'FileUploader'];
function addicsfileCtl($scope, $filter, $stateParams, icsdb, $state, SweetAlert, eventorganization, FileUploader) {
    $scope.typeList = ['应急培训要求', '培训记录', '主要人员应急能力', '应急预案', '处置方案', '应急演练记录', '其他'];
    $scope.selType = $scope.typeList[0];
    $scope.fList = [];
    $scope.init = function () {
    ...
    $scope.uploader.onSuccessItem = function (fileItem, response, status, headers) {
        ...
        var obj = {
            institutions: $scope.selOrg.id,//组织机构
            content: response.filename.split('"').join(''),
            type: $scope.selType,
