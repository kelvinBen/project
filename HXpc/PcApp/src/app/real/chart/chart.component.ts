...
export class ChartComponent implements OnInit {
  ...
  config = { format: 'YYYY-MM-DD HH', locale: moment.locale('zh-CN') };
  option: any;
  ...
  queryFactor() {
    const endTime = moment(this.eDate.valueOf(), 'YYYY-MM-DD HH').add(-8, 'hours').format('YYYYMMDDHH') + '0000';
    const startTime = moment(this.sDate.valueOf(), 'YYYY-MM-DD HH').add(-8, 'hours').format('YYYYMMDDHH') + '0000';
    ...
    parUrl += 'TEM,TEM_Min,TEM_Max,WIN_S_Inst_Max,WIN_D_INST_Max,VIS_Min,WIN_D_Avg_2mi,WIN_S_Avg_2mi';
    ...
    this._http.get(parUrl).subscribe(result => {
      ...
      if (dt && dt.returnCode === '0') {
        ...
        infos.forEach(item => {
          ...
          this.arr_windd.push([strTime,
            (item.WIN_S_Avg_2mi > '9999' ? 0 : item.WIN_S_Avg_2mi),
            (item.WIN_D_Avg_2mi > '9999' ? 0 : item.WIN_D_Avg_2mi)]);
          this.arr_instantwindv.push(item.WIN_S_Inst_Max > '9999' ? 0 : item.WIN_S_Inst_Max);
          this.arr_windv.push(item.WIN_S_Avg_2mi > '9999' ? 0 : item.WIN_S_Avg_2mi);
  ...
  createOption(): any {
    return {
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          for (let i = 0; i < params.length; i++) {
            ...
            if (params[i].seriesName === '2分钟平均风' && params[i].seriesType === 'custom') {
              relVal += '2分钟平均风：' + '风速' + params[i].data[1] + 'm/s' + ' , ' + '风向' + params[i].data[2] + '°' + '<br/>';
            }
          } return relVal;
        }
      },
      ...
      calculable: true,
      legend: {
        data: ['温度', '最高温度', '最低温度', '极大风', '2分钟平均风', '相对湿度', '气压', '降水量'],
        x: 'center',
        selected: {
          '极大风': false,
          '相对湿度': false,
